...
public class LevelGeneratorWindow : EditorWindow
{
    ...
    private float spacing = 1.0f; // Khoảng cách giữa các tile cùng layer
    ...
    private void OnGUI()
    {
        ...
        spacing = EditorGUILayout.FloatField("Grid Spacing (X, Y)", spacing);
    ...
    private void GenerateLevel()
    {
        ...
        string folderPath = "Assets/_Project/Resources/SO";
    ...
    private List<LevelTileData> GenerateLayout()
    {
        ...
        for (int layer = 0; layer < maxLayers; layer++)
        {
            ...
            float startX = -(baseWidth - 1) * spacing / 2f + (layer * layerOffsetX);
            float startY = -(baseHeight - 1) * spacing / 2f + (layer * layerOffsetY);
    ...
    {
        // Lấy danh sách các loại bài (Từ Sword đến Coin)
        List<CardType> availableTypes = new List<CardType>();
        for (int i = 1; i <= 13; i++) 
        {
            availableTypes.Add((CardType)i);
        }
