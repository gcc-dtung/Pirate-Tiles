...
| **Communication** | **Hybrid: EventBus (static generic) + Event Channel SO (ScriptableObject-based)** |
| **Animation** | DOTween |
...
## 6. Hệ Thống Giao Tiếp — Hybrid

Dự án sử dụng **kiến trúc hybrid** kết hợp 2 hệ thống event:

| | EventBus (Static Generic) | Event Channel SO (ScriptableObject) |
|---|---|---|
| **Khi dùng** | Sự kiện nội bộ, system-level, tần suất cao | Sự kiện cross-layer, cần Inspector debug |
| **Ví dụ** | GamePhaseChanged, TileStateChanged, SaveDataChanged | TileSelected, GameWon, TimerExpired |
...
> Chi tiết tại `_02.Architecture.md` section 2.
...
| File | Nội dung |
|---|---|
| `_02.Architecture.md` | Kiến trúc MVC chi tiết, thiết kế các layer, hệ thống event hybrid |
...
## 8. Điểm Khác Biệt So Với Bản Gốc (Wonder Match)

| Mục | Wonder Match (gốc) | Pirate Tiles (mới) |
|---|---|---|
| **Theme** | Hoa lá, bộ bài poker | 🏴‍☠️ Cướp biển, đảo kho báu |
| **Communication** | `EventBus` (static class only) | **Hybrid: EventBus + Event Channel SO** |
...
| **Kiến trúc** | MVC + EventBus only | MVC + **Hybrid Event System** |
| **Gameplay** | Giữ nguyên 100% | Giữ nguyên 100% |

---

> **Ghi chú:** Pirate Tiles giữ nguyên **toàn bộ cơ chế gameplay** của Wonder Match. Sự khác biệt chính nằm ở **theme cướp biển** và việc chuyển sang **hệ thống event hybrid** — kết hợp EventBus (cho sự kiện nội bộ, system-level) với Event Channel SO (cho sự kiện cross-layer, cần Inspector debug).
